...
public:
    long long countGoodSubarrays(vector<int>& nums) {
        ...
        const auto& is_subset_or_equal_to = [](int a, int b) {
            return (a | b) == b;
        };
        ...
        for (int i = size(nums) - 1; i >= 0; --i) {
            for (; !empty(stk) && is_subset_or_equal_to(nums[stk.back()], nums[i]); stk.pop_back());
...
public:
    long long countGoodSubarrays(vector<int>& nums) {
        ...
        for (int i = 0; i < size(nums); ++i) {
            for (; !empty(stk) && !is_subset_or_equal_to(nums[i], nums[stk.back()]); stk.pop_back()) {
                right[stk.back()] = i;
            }
            stk.emplace_back(i);
        }
