...
public:
    int minAbsDifference(vector<int>& nums, int goal) {
        ...
        vector<int> sorted_sums1(cbegin(sums1), cend(sums1));
        sort(begin(sorted_sums1), end(sorted_sums1));
        ...
        for (int i = size(nums) / 2; i < size(nums); ++i) {
            for (const auto& x : vector<int>(cbegin(sums2), cend(sums2))) {
                ...
                auto cit = lower_bound(cbegin(sorted_sums1), cend(sorted_sums1), goal - x - nums[i]);
