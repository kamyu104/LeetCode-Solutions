...
class Solution(object):
    def canWin(self, s):
        ...
        i, n = 0, len(s) - 1
        other_win = True
        while other_win and i < n:                                     # O(n) time
            if s[i] == '+':
                while other_win and i < n and s[i+1] == '+':           # O(c) time
                     # F(n, c) = n * (F(n, c-1) + n) = ... = n^c * F(n, 0) + n^2 * (1 + n + ... + n^(c-1))
                     # = n^(c+1) + n^2 * (n^c - 1) / (n - 1) = O(n^(c+2))
                    other_win = self.canWin(s[:i] + '--' + s[i+2:])    # O(n) space
                    i += 1
            i += 1
        return not other_win
