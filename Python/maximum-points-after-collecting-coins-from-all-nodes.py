    ...
    def maximumPoints(self, edges, coins, k):
        ...
        def dfs(u, p, base):
            ...
            return max(((coins[u]//base)-k)+sum(dfs(v, u, base) for v in adj[u] if v != p),
                        (coins[u]//(base<<1))+sum(dfs(v, u, base<<1) for v in adj[u] if v != p) if (coins[u]//base)-k < coins[u]//(base*2) else 0)  #  if (coins[u]//base)-k >= coins[u]//(base*2), the first way is always better
