...
class Solution(object):
    def minimumSum(self, grid):
        ...
        def count2():
            def horizon():
                dp = [[0]*len(grid) for _ in xrange(len(grid))]
                for i in xrange(len(grid)):
                    l, r, u, d = len(grid[0]), -1, len(grid), -1
                    for j in xrange(i, len(grid)):
                        if right[j] != -1:
                            l = min(l, left[j])
                            r = max(r, right[j])
                            u = min(u, j)
            ...
            def vertical():
                dp = [[0]*len(grid[0]) for _ in xrange(len(grid[0]))]
                for i in xrange(len(grid[0])):
                    l, r, u, d = len(grid[0]), -1, len(grid), -1
                    for j in xrange(i, len(grid[0])):
                        if down[j] != -1:
                            l = min(l, j)
                            r = max(r, j)
                            u = min(u, up[j])
            ...
            left = [len(grid[0]) for _ in xrange(len(grid))]
            right = [-1 for _ in xrange(len(grid))]
            up = [len(grid)]*len(grid[0])
            down = [-1]*len(grid[0])
            for i in xrange(len(grid)):
